...
function FolderIcon({ label, type, onDoubleClick, onContextMenu, theme, position, onMove, onDrop, isRenaming, onRenameSubmit, onRenameCancel, inputRef, onDragStart }: FolderIconProps) {
  ...
  const handleMouseDown = useCallback(
    ...
    [position, onMove, onDrop, isRenaming],
  );
    ...
    (e: React.TouchEvent) => {
      ...
      const onUp = (ev: TouchEvent) => {
        ...
        if (!el.moved) {
          ...
          onDrop(el.startPosX + t.clientX - el.startX, el.startPosY + t.clientY - el.startY);
        }
      };
      document.addEventListener("touchmove", onDrag, { passive: false });
      document.addEventListener("touchend", onUp);
    },
    [position, onMove, onDrop, isRenaming, onDoubleClick],
  );
